...
## 주석(comment)
> 부연 설명을 위해 사용되는 주석은 컴파일러가 무시한다.<br>
> Rust 의 주석은 두개의 슬래시(//)로 시작되며 줄의 끝까지 게속된다.<br>
> 코드 윗줄에 따로 주석을 작성한 것이 보기 좋다.<br>

## if문
> 조건에 따라 분기할 수 있도록 해준다.<br>
> if 조건식 {} 으로 사용하며, 조건식이 참일 경우 중괄호 블록을 실행하고 거짓일 경우 else 표현식이 실행된다. else 가 없을 경우 if 블록문은 생략된다.<br>
> 조건식은 반드시 bool 이여야 한다.<br>
> Rust 는 bool 타입으로 자동 변환하지 않는다. 그러므로 if 문에는 항상 명시적으로 bool 타입의 조건식을 제공해야 한다.<br>

## else if 다중 조건식
> if 와 else 사이에 else if 를 조합하여 여러 조건식을 사용할 수 있다.<br>
> 각각의 if 표현식을 순차적으로 검사하고 조건이 참인 첫 번째 본문을 실행한다.<br>
> else if 표현식을 많이 사용하면 코드가 복잡해지므로 2개 이상인 경우 리팩토링이 추천된다.<br>

## let 구문에서 if 문 사용
> let 구문의 우변으로 if 조건식 { 참일때 할당할 값인 표현식 } else { 거짓일때 할당할 값인 표현식 }; 으로 사용할 수 있다.
> 이떄, 표현식의 타입은 참일때와 거짓일때 모두 같아야 한다.

## 반복문 - loop
> Rust 는 loop 키워드를 이용하여 블록 내 코드들을 반복할 수 있다.<br>
> 반복 종료시 연산 결과값을 전달할려면 break 표현식 뒤에 반환하고자 하는 값을 넣으면 된다.<br>
> 루프에 루프 라벨(loop label)을 명시적으로 추가시, break 나 continue 와 함께 바깥쪽 루프 대신, 라벨이 적힌 특정한 루프에 적용되도록 할 수 있다.<br>
> 루프 라벨은 반드시 작은따옴표(')로 시작해야 한다.<br>
> 무한 루프 구조에 사용된다.

## 반복문 - while
> 반복문 내 조건문이 true 인 동안에는 계속 반복하고, true 가 아닐게 될 때 break 를 호출하여 반복을 종료한다. 이를 loop, if, else, break 조합으로 구현할 수 있다.<br>
> 그러나 while 반복문을 이용하면 하나의 키워드로 깔끔하게 출력할 수 있다. 조건식이 true 로 계산되는 동안 반복 실행되고, 그렇지 않으면 반복문을 종료한다.<br>
> 반복 조건이 명확할 때 사용된다.

## 반복문 - for
> 컬렉션의 각 요소에 대한 반복문을 작성할 때 for 키워드가 사용된다.<br>
> for 패턴변수 in (범위) {} 구조로 구성되며, 범위는 항상 시작값 이상, 끝값 미만 구조를 기본값으로 가진다.<br>
> 범위에서 (시작값..끝값) 은 시작값 이상 끝값 미만 이고 (시작값..=끝값) 은 시작값 이상 끝값 이하 이다.<br>
> 범위에서 밑줄(_) 을 이용하여 값을 무시하고 반복만 시킬 수 있다.<br>
