...
> 만약 Copy 트레이트를 구현한 타입만 이동될 경우 기존 인스턴스를 사용할 수 있다.<br>

## 튜플 구조체(tuple struct)
> 튜플 구조체는 구조체 자체는 이름을 지어 의미를 주지만, 이를 구성하는 필드에는 이름을 붙이지 않고 타입만 적어 넣음<br>
> 특정 튜플을 다른 튜플과 구분하거나, 각 필드명을 일일이 정하기 불편할 경우 유용하다.<br>
> struct 구조체명(타입1, 타입2); 으로 이뤄진다.<br>
> 서로 다른 튜플 구조체는 필드 구성이 같더라도 타입이 다르므로 별개 취급이다.

## 유사 유닛 구조체(unit-like struct)
> 유사 유닛 구조체는 어떤 타입에 대해 트레이트를 구현하고 싶을때, 타입 내부에 어떤 데이터를 저장할 필요가 없는 경우에 유용하다.<br>
```
struct AlwaysEqual;
...
```
> struct 키워드와 구조체 이름만 선언한다. 해당 구조체의 모든 인스턴스는 언제나 다른 모든 타입의 인스턴스와 같도록 하는 동작을 구현하며, 데이터가 필요없는 이미 알고 있는 결과값의 테스트 용도를 사용한다.
