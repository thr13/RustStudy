...
## 댕글링 참조
> 댕글링 포인터(dangling pointer)란, 어떤 메모리를 가르키는 포인터가 남아있는 상황에서 일부 메모리를 해제하여 다른 개체가 할당받았을지도 모르는 메모리를 참조하게 된 포인터를 의미한다.<br>
> Rust 는 어떤 데이터의 참조자를 만들면, 해당 참조자가  스코프를 벗어나기 전, 데이터가 먼저 스코프를 벗어나는지 컴파일러에서 확인해 댕글링 참조가 생성되지 않도록 보장한다.<br>

## 참조자 규칙
> 단 하나의 가변 참조자만 갖거나, 여러 개의 불변 참조자를 가질 수 있다.<br>
> 참조자는 항상 유효해야 한다.<br>

## 슬라이스 타입
> 슬라이드(slice)는 컬렉션(collection)을 통째로 참조하는게 아닌, 컬렉션의 연속된 일련의 요소를 참조하도록 해준다.<br>
> 슬라이는 참조자의 일종으로 소유권을 갖지 않는다.

## 문자열 슬라이스(string slice)
> 문자열 슬라이스는 String 의 일부를 가리키는 참좌를 의미한다.<br>
> &참조변수[시작인덱스..끝인덱스] 으로 구성되며 시작 인덱스 부터 끝 인덱스-1 뺀 위치까지 슬라이스를 생성한다.<br>
> 인덱스 0 부터 시작할 경우 앞의 값을 생략할 수 있다. 마찬가지로 String 맨 마지막 바이트까지 포함하는 슬라이스는 뒤의 값을 생략할 수 있다.<br>
> 문자열 리터럴은 곧 문자열 슬라이스이므로, 슬라이스 문법 없이 작동한다.<br>
> 문자열 일부를 참조할 때 처럼 배열 일부를 참조하고 싶다면 &[시작인덱스..끝인덱스] 로 사용할 수 있다.
